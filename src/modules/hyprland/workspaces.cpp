...
std::string Workspaces::getRewrite(std::string window_class, std::string window_title) {
  // Window rewrite is no longer used for icons - we only use GTK system icons
  // This is kept for backward compatibility but returns empty string
  // Icons are now rendered as GTK Image widgets, not text
  return "";
}
...
auto Workspaces::parseConfig(const Json::Value &config) -> void {
  ...
  // Parse enhanced workspace features
  populateBoolConfig(config, "deduplicate-windows", m_deduplicateWindows);
  populateBoolConfig(config, "use-system-icons", m_useSystemIcons);
  populateBoolConfig(config, "show-workspace-number", m_showWorkspaceNumber);
  populateBoolConfig(config, "show-special-workspace-number", m_showSpecialWorkspaceNumber);

  const auto& workspaceGroupingConfig = config["workspace-grouping"];
  if (workspaceGroupingConfig.isString()) {
    m_workspaceGrouping = workspaceGroupingConfig.asString();
  }

  const auto& iconSizeConfig = config["icon-size"];
  if (iconSizeConfig.isInt()) {
    m_iconSize = iconSizeConfig.asInt();
  }

  const auto& specialIndicatorConfig = config["special-workspace-indicator"];
  if (specialIndicatorConfig.isString()) {
    m_specialWorkspaceIndicator = specialIndicatorConfig.asString();
  }

  const auto& specialScaleConfig = config["special-workspace-icon-scale"];
  if (specialScaleConfig.isDouble() || specialScaleConfig.isNumeric()) {
    m_specialWorkspaceIconScale = specialScaleConfig.asDouble();
  }
...
void Workspaces::sortWorkspaces() {
  // Handle paired workspace grouping mode
  if (m_workspaceGrouping == "paired") {
    sortWorkspacesPaired();
