...
auto waybar::Bar::setupWidgets() -> void {
  ...
  for (auto const& module : modules_left_) {
    // Check if widget already has a parent before adding
    Gtk::Widget& widget = *module;
    auto* parent = widget.get_parent();
    if (parent != nullptr) {
      auto* container = dynamic_cast<Gtk::Container*>(parent);
      if (container != nullptr) {
        container->remove(widget);
      }
    }
    left_.pack_start(widget, module->expandEnabled(), module->expandEnabled());
  }
  ...
  if (!no_center) {
    for (auto const& module : modules_center_) {
      ...
      center_.pack_start(widget, module->expandEnabled(), module->expandEnabled());
    }
  }
